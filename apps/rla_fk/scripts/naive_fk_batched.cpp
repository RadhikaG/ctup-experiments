...
static void set_X_T(const Model &model, blaze::DynamicVector<XformBatched_t> &X_T) {
  size_t r = 0;
  size_t c = 0;
...
static XformBatched_t fk(const Model &model, blaze::DynamicVector<XformBatched_t> &X_T, blaze::DynamicVector<blazeSIMDVec8d> &q) {
  int jtype;
  int axis;

  blaze::DynamicVector<XformBatched_t> X_J(model.njoints);
  blaze::DynamicVector<XformBatched_t> X_0(model.njoints);

  blazeSIMDVec8d sinq, cosq;
...
{
  argparse::ArgumentParser program("naive_fk_batched");
  ...
  blaze::DynamicVector<blazeSIMDVec8d> blaze_q;
  ...
  blaze::DynamicVector<XformBatched_t> X_T(model.njoints);
  set_X_T(model, X_T);
