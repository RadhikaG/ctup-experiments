...
builder::dyn_var<void (
    ...
    ctup::EigenMatrixXd & // signed_distances (modified in-place)
        )> compute_sph_sph_cp_sd = builder::as_global("cg_sd_runtime::compute_sph_sph_cp_sd");
}
...
// computes signed distances 8 robot configurations at once (batch size = 8)
// returns:
// distances: shape: (batch_size, n_collision_pairs)
template <typename RobotT>
static void self_collision_signed_distances(
    const pinocchio::Model &model, 
    const pinocchio::GeometryModel &geom_model, 
    ...
    dyn_var<ctup::EigenMatrixXd&> signed_distances)
{
  ...
  for (i = 0; i < (JointIndex)model.njoints; i = i+1) {
    ...
    //
    // in code: 
    // [x,y,z]_0[link_id] = X_0 * link_spheres[link_id]
    // But, we only care about the translation component so:
    // [x,y,z]_0[link_id].trans = X_0.rot * link_spheres[link_id].trans + X_0.trans
    ...
    for (static_var<size_t> cs_i = 0; cs_i < joint_child_spheres.r.size(); cs_i = cs_i+1) {
      ...
      for (static_var<size_t> cp_it = 0; cp_it < geom_model.collisionPairs.size(); cp_it = cp_it+1) {
        ...
        if (cp.second == curr_geom_id) {
          ...
          // 2. Slots the 8 SD values generated back into the appropriate spot in the 
          //    `signed_distances` array modified in-place.
          runtime::compute_sph_sph_cp_sd(
              cp_it,
...
int main(int argc, char* argv[]) {
  argparse::ArgumentParser program("batched_self_collision_sd");
  ...
  if (robot_name == "panda") {
    ...
    auto ast = context.extract_function_ast(
            self_collision_signed_distances<runtime::robots::Panda>, 
            "self_collision_signed_distances", 
            model, geom_model);
